...
export default function Edit({ empleadoUser, users, empleados }: { 
    empleadoUser: any, 
    users: any[], 
    empleados: any[] 
}){
    const { data, setData, put, processing, errors } = useForm({
        id_user: empleadoUser.id_user?.toString() || '',
        id_empleado: empleadoUser.id_empleado?.toString() || '',
    });

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(`/admin/empleado-users/${empleadoUser.id}`);
    };

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Editar Asignación Empleado-Usuario" />
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-muted-foreground mt-2">
                            Actualiza la asignación de empleado a usuario
                        </p>
                    </div>
                </div>
